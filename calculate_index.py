...
DB_FILE = str(BASE_DIR / "airfare_intelligence.db")
JSON_OUTPUT = str(BASE_DIR / "dashboard_data.json")
CSV_OUTPUT = str(BASE_DIR / "calculated_airfare_index.csv")
...
MOSPI_CPI_SERIES = [102.10, 103.40, 104.20, 105.10, 105.63]
...
def run_laspeyres_engine():
    ...
    for corridor, weight in CORRIDORS_8.items():
        ...
        surge_1d = h.get(1, current_price * 1.25)
    ...
    national_index = round(composite_index / total_weight, 2) if total_weight > 0 else 100.0

    # Build empirical NAPI series across horizons
    napi_series = []
    for h in [45, 30, 15, 7, 1]:
        if h_fares[h]:
            mean_h = sum(h_fares[h]) / len(h_fares[h])
            base_ref = sum(h_fares[30]) / len(h_fares[30]) if h_fares[30] else 5400.0
            napi_series.append(round((mean_h / base_ref) * 105.0, 2))
        else:
            napi_series.append(100.0 + (45 - h) * 0.15)

    macro_score = calc_pearson(napi_series, MOSPI_CPI_SERIES)
    ...
    conn.close()
    print(f"[✔] Laspeyres Index: {national_index} | Dynamic Macro Pearson Score: {macro_score}")
