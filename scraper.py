...
Physically interacts with the Google Flights UI to extract real Economy, Premium Economy, Business, and First Class fares.
Enforces an 8-hour ceiling and rejects unidentified airlines to eliminate 'Scheduled Airline' placeholders.
"""
...
async def parse_flight_card(card, category, origin, dest, dep_date, advance_days, cabin, current_time_iso, idx):
    try:
        ...
        # 3. Duration & Layover Filter (Reject > 8 hours to avoid overnight layovers)
        dur_match = re.search(r"(\d+)\s*hr(?:\s*(\d+)\s*min)?", card_text)
        if dur_match:
            hrs = int(dur_match.group(1))
            if hrs > 8:
                return None  # Discards extreme layovers while keeping valid 5-7.5 hr flights
            mins = dur_match.group(2)
        ...
        # Discard unrecognized airlines instead of writing "Scheduled Airline"
        if not airline or airline == "Scheduled Airline":
            return None
